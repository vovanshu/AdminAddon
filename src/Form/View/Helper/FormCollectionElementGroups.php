<?php
namespace AdminAddon\Form\View\Helper;
...
class FormCollectionElementGroups extends FormCollection
{
    ...
    public function render(ElementInterface $element): string
    {
        ...
        foreach ($elementGroups as $elementGroupName => $elementGroupLabel) {
            ...
            $groupName = $view->escapeHtml('fieldset-'.$elementGroupName);
            $markup .= sprintf('<fieldset name="%s" id="%s" aria-labelledby="%s">', $groupName, $groupName, $groupName . '-label');
            $markup .= sprintf('<legend id="%s-label">%s</legend>', $groupName, $view->escapeHtml($view->translate($elementGroupLabel)));
